...
export default function DashboardPage() {
  ...
  return <Shell><div className="page"><header className="page-header"><div><span className="eyebrow">Overview</span><h1>Dashboard</h1><p>Real-time analytics and telemetry across your Vermex workspace.</p></div></header>{error ? <div className="alert">{error}</div> : null}{!data ? <Loading /> : <><section className="metric-grid">{metrics.map(([key, label, Icon]) => <article className="metric" key={key}><span>{createElement(Icon, { size: 18 })}</span><small>{label}</small><strong>{Number(data.metrics[key] || 0).toLocaleString()}</strong></article>)}</section><section className="dashboard-grid"><article className="surface"><div className="section-title"><div><span className="eyebrow">Recent</span><h2>Conversations</h2></div></div>{data.recent.length ? data.recent.map((item) => <div className="list-row" key={item.id}><span className="row-icon"><MessageSquare size={16} /></span><div><strong>{item.title}</strong><small>{new Date(item.updatedAt).toLocaleString()}</small></div></div>) : <p className="muted">Start a conversation to see activity here.</p>}</article><article className="surface"><div className="section-title"><div><span className="eyebrow">Execution</span><h2>Agent activity</h2></div></div>{data.agents.length ? data.agents.map((item) => <div className="list-row" key={`${item.agent}-${item.status}`}><span className="row-icon"><Bot size={16} /></span><div><strong>{item.agent}</strong><small>{item.status}</small></div><b>{item.value}</b></div>) : <p className="muted">Agent telemetry appears after task execution.</p>}</article></section></>}</div></Shell>;
}
